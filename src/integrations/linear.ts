...
export class LinearIntegration extends BaseIntegration {
  ...
  private async createIssue(data: BugReportData, config: LinearConfig) {
    ...
    const triageStatus = await this.getTriageStatus(config);
    ...
    const variables = {
      input: {
        ...
        ...(triageStatus && { stateId: triageStatus }), // Use triage status if available
  ...
  private async getTriageStatus(config: LinearConfig): Promise<string | null> {
    try {
      const query = `
      ...
      if (result.errors || !result.data?.team?.states?.nodes) {
        console.warn('Could not fetch team states, falling back to default behavior');
        return null;
      }
      ...
      console.warn('Failed to fetch team states:', error);
      return null;
    }
  }
